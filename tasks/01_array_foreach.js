...
const findHigherSalariedEmployees = (employees = [{name: '', salary: 200000}]) => {
    // 
    const employeesWithHigherSalary = [];
    employees.forEach( e => {
        if (e.salary > 300000) {
            employeesWithHigherSalary.push(e);
        }
    })
...
const findEmployeesFromMarketing = (employees=[{name: '', salary: 200000, dep: 'marketing'}]) => {
    ...
    employees.forEach( e => {
        if (e.salary > 300000 && e.dep === 'marketing') {
            marketingEmployees.push(e);
        }
    })
...
const findGermanMigrants = (citizens=[{nation: 'German', email: 'franz@gmail.com', city: 'Budapest'}]) => {
    ...
    const migrants = [];
    citizens.forEach( e => {
        if (e.city === 'Budapest' && e.nation === 'German' && e.email.includes('gmail')) {
            migrants.push(e);
        }
    })
    return migrants;
