...
const transformTitleCase = (foods = ['milk', 'bread', 'apple']) => {
    // 
    return foods.map( f => f[0].toUpperCase() + f.slice(1) );
}
...
const addNamePrefixToStudents = (students = [{lastName: 'Doe', gender: 'male'}]) => {
    ...
    return students.map( s => {
        const prefix = s.gender === 'male' ? 'Mr.' : 'Ms.';
        s.lastName = `${prefix} ${s.lastName}`;
        return s;
    });
}
...
const getStudentReports = (students = [{firstName: 'John', lastName: 'Doe', points: 20}]) => {
    return students.map( s => `${s.firstName} ${s.lastName}, points: ${s.points}.`);
}
// törölni
...
const addStudentReports = (students = [{firstName: 'John', points: 20}]) => {
    return students.map( s => ({...s, result: s.points > 50 ? 'passing' : 'not passing'}));
}
// törölni
...
const addCorrectedIncome = (employees=[{name: 'Bob', salary: 200000, premium: 1000, corr: 0.2}]) => {
    return employees.map( e => ({...e, income: (e.salary + e.premium) * (1 + e.corr)}) );
}
// törölni
...
const addDishPrices = (dishes=[{id: 1, kcal: 101, type: 'dessert'}]) => {
    return dishes.map( d => {
        ...
        if (d.type === 'dessert') {
            price = Math.round(200 - d.kcal * 1.5);
        } else {
            price = Math.round(25 + d.kcal * 1);
        }
